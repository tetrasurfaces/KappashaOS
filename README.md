...
- **`hardware/proto/`**: Hardware specifications and drivers:  
  ...
  - `KappashaOS`: Monoscript snapshot of repo commit history and file contents (no extension), generated by `repo_audit.py`, kept private for legal audit. Regenerate with `repo_audit.py` and re-hash with `kappa_hash.py` after updates to ensure integrity.  
- **`software/proto/`**: Experimental software components:  
  ...
  - `corneal_etch.py`: Simulates a 0.2-micron waveguide etch on fused-silica cornea.  
  - `automaton_pie.py`: Simulates a 2mm sapphire piezo-optic interface for nerve coupling.  
