...
def kappa_spiral(theta, laps=18, ratio=1.618):  # Cosmic bud golden ratio
    phi = (1 + np.sqrt(5)) / 2
...
def cone_braid(laps=54, strands=55, delays=[0.11, 0.55, 1.1], video_path="/home/username/KappashaOS-main/your_video.mp4"):
    ...
    # Batch Kappa Ys for gentleness
    user_ids = np.arange(0, 107, 10)  # Batch of 11 points
    thetas = np.linspace(0, np.mean(t), len(user_ids))
    points_batch = np.array([kappa_coord(uid, theta) for uid, theta in zip(user_ids, thetas)]) / 1023
    points = np.vstack((np.array([[0, 0, 0]]), points_batch))  # 0x0 null + batch
    ...
    if heat_spike():
        print("Navi: Hush—pausing weave due to heat spike.")
        return False, 0.0, []
