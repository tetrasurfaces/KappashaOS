...
mpmath.mp.dps = 19
PHI = float(mpmath.phi)  # ≈1.618033988749895

@boundscheck(False)
@wraparound(False)
cpdef double compute_phi_kappa(double[:, :] points):
    """PHI-Scaled Kappa Computation (19 decimals precision)."""
    cdef int n = points.shape[0]
    if n < 3 or points.ndim != 2 or points.shape[1] != 2:
        return 0.0  # or raise ValueError("Expected (N,2) array")
    ...
    # Use np.zeros — safe coercion to memoryview
    cdef double[:] kappa = np.zeros(n-2, dtype=np.float64)

    cdef int i
    cdef double denom

    for i in range(n-2):
        denom = (dl[i]*dl[i] + dh[i]*dh[i]) ** 1.5
        if denom == 0.0:
            kappa[i] = 0.0
        else:
