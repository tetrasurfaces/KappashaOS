...
def test_phi_kappa():
    # Simple semicircle-like points (x,y)
    theta = np.linspace(0, np.pi, 20)
    points = np.column_stack((np.cos(theta), np.sin(theta)))  # radius 1 circle arc

    kappa = compute_phi_kappa(points)
    print(f"Mean phi-scaled curvature: {kappa:.12f}")
    # For circle radius 1 → curvature should be ~1.0 (phi-scaled ≈1.618)
