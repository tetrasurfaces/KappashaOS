...
def expand_hash_grid(exponents, size=10):
    ...
    for i, p in enumerate(exponents[:size**2]):
        grid.flat[i] = p + np.random.uniform(0, 0.1)  # Add decimal jitter
    # Negative symmetry: mirror with negative
    neg_grid = -grid
...
if __name__ == "__main__":
    ...
    print("Hash grid sample:\n", grid[:5, :5])
    plt.imshow(grid, cmap='viridis')
    plt.title("Expanded Hashing Grid with Mersenne")
    plt.show()
    # For large p, e.g., 136279841, too big for full mp, but for grid seed ok
    large_p = 136279841
    print(f"Largest known p={large_p}, mp too large to compute full.")
