...
async def kappa_transform(state, key_lanes, round_idx, prime_index):
    for x in range(GRID_DIM):
        for y in range(GRID_DIM):
            ...
            state[x][y] ^= ((key_lanes[x][y] >> shift) & ((1 << LANE_BITS) - 1)) * weight
...
async def iota(state, round_idx):
    RC = [  # Mersenne-modulated round constants
        (0x0000000000000001 + round_idx * 3) % (1 << 64),
        (0x0000000000008082 + round_idx * 7) % (1 << 64),
        # ... add more, modulated by mersenne primes
    ] * (ROUNDS // len(RC))  # placeholder, expand later
...
async def absorb(state, chunk, key_lanes):
    i = 0
    for x in range(GRID_DIM):
        for y in range(GRID_DIM):
            if i < len(chunk):
                state[x][y] ^= int.from_bytes(chunk[i:i+8], 'little') ^ key_lanes[x][y]
                i += 8
            await asyncio.sleep(0)
    return state
...
async def secure_hash_zero(message, key, prime_index=11):
    ...
    for i in range(0, len(padded), rate_bytes):
        ...
        state = await absorb(state, chunk, key_lanes)
    ...
    # Ternary output mapping
    ternary = int(hash_hex[:2], 16) % 3  # simple mock
    colors = ["gray", "gold", "zero"]
    braided = f"{hash_hex[:16]}:{colors[ternary]}"
    return hash_hex, flattened, quotient, braided

# Yellowstone single-swap test
async def yellowstone_test():
    message = b"test message"
    key = hashlib.sha256(b"secret").digest() * 2
    original_hex, orig_flat, orig_quot, orig_braid = await secure_hash_zero(message, key)
    # Single byte swap in message
    swapped_msg = bytearray(message)
    swapped_msg[0] ^= 0x01  # flip bit 0
    swapped_hex, swap_flat, swap_quot, swap_braid = await secure_hash_zero(bytes(swapped_msg), key)
    diff = abs(orig_flat - swap_flat)
    print(f"Original flat: {orig_flat}")
    print(f"Swapped flat: {swap_flat}")
    print(f"Yellowstone ripple diff: {diff}")

if __name__ == "__main__":
    asyncio.run(yellowstone_test())
