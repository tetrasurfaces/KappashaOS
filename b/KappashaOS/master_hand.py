...
from blockclockspeed_fleet import BlockclockspeedFleet
from src.core.kappa_factory import create_kappa
from src.core.kappaendian_base import KappaEndianBase
...
class MasterHand:
    ...
    def pulse(self, level=1):
        self.vibe_model.pulse(level)
        print(f"Master Hand pulses at level {level}")
    ...
    def log_hash(self, hash_val):
        """Log hash to echo and gossip queue."""
        try:
            short = str(hash_val)[:16]
            self.echo.record(f"hash {short}")
            self.gossip_queue.put(short)
            #print(f"Master Hand logged hash {short}")
        except Exception as e:
            print(f"Log hash flinch: {e}")
