...
for row in range(GRID_SIZE):
    ...
    splines.append(c.copy())  # .copy() important!
...
def encode_char(char):
    ...
    for r in range(GRID_SIZE):
        for c in range(GRID_SIZE):
            if grid[r, c] == '.':
                grid[r, c] = char
                h = hashlib.sha256(f"{r}{c}{char}{note}".encode()).hexdigest()  # chain with previous note for security
                hash_chain.append(h)
                print(f"Encoded '{char}' at ({r},{c}) | hash: {h[:8]}...")
                modulate(h)
                note += char
                return r, c
    print("Grid full!")
...
def decode_note():
    print("\n=== DECODING ===")
    # Reset splines to initial state
    temp_grid = np.full((GRID_SIZE, GRID_SIZE), '.', dtype='<U1')
    temp_splines = []
    for row in range(GRID_SIZE):
        temp_splines.append(np.array([
            [0.0, float(row)],
            [1.0, float(row)],
            [3.0, float(row)],
            [4.0, float(row)]
    ...
    recovered = ""
    for h in hash_chain:
        modulate(h)  # apply same warps in order
        # Scan grid for non-'.' that hasn't been recovered yet
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                ch = grid[r, c]
                if ch != '.' and ch not in recovered:
                    recovered += ch
                    print(f"Recovered '{ch}' (likely from warped position near ({r},{c}))")
                    break
            else:
                continue
            break
...
print("\nFinal encoded grid + warped splines:")
show()

print(f"\nTrue note: '{note}'")
print(f"Hash chain: {[h[:8] for h in hash_chain]}")

print("\n--- DECODING ---")
recovered = decode_note()
print(f"Recovered note: '{recovered}'")
print(f"Success: {recovered == note}")