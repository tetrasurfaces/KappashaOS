...
async def hxsh(my_hash, their_hash):
    ...
    await x_client.post_tweet(f"#hxsh {my_hash} {their_hash}")
    await x_client.send_dm(their_id="THEIR_ID", message=f"call:{key_b64}")

    webrtc = WebRTC(key=key, my_hash=my_hash, their_hash=their_hash)
    webrtc.start()

    while True:
        key = await hal9001.ramp_key(key)  # Ramp cipher from HAL9001
